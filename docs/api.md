...
如果配置 `VISION_UNDERSTANDING_MODEL`、`VISION_UNDERSTANDING_BASE_URL` 和 `VISION_UNDERSTANDING_API_KEY`，图片找货会先调用 OpenAI-compatible VLM，把图片理解为本地商品库可用的品类、子类目、关键词和属性；这些信号只参与召回/排序，不会生成商品事实。
