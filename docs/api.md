...
- `stock_status`、`inventory_count`：演示用库存状态与库存数，下单前会校验。
...
- `compare`：结构化对比结果。
- `cart`：购物车状态。
- `order`：Agent 引导确认地址并模拟下单后的结构化订单，包含订单号、地址、商品明细、总价和支付状态。
...
加购、改数量和下单都会校验 `inventory_count` 与 `stock_status`。如果库存不足，接口返回错误，Agent 不会继续生成订单。
...
对话式闭环也走同一套工具：用户说“下单/结算”后，Agent 会汇总购物车、要求收货地址、等待“确认下单”，最终发送 `order` SSE 事件并清空购物车。
