...
2. `ProductRepository._matches_constraints` 做硬过滤：
   - 类目、子类目
   - 最高价、最低价
   - 包含偏好
   - 排除词、排除品牌
3. 对剩余商品做向量相似检索：优先使用 `TEXT_EMBEDDING_*` 配置的真实 embedding，未配置或调用失败时自动回退到本地 Hashing 向量。
4. 使用结构化命中进行加权，例如品牌、子类目、包含偏好。
5. 返回前 N 个商品，并生成 grounded reason。
...
VISION_UNDERSTANDING_IMAGE_DETAIL=auto
VISION_UNDERSTANDING_MAX_IMAGE_SIDE=1024
VISION_UNDERSTANDING_MAX_TOKENS=300
VISION_UNDERSTANDING_JSON_MODE=false
```

真实调参通过 `server/scripts/probe_vision_understanding.py` 完成。常用策略是先用 `detail=auto`、最长边 1024 验证识别词是否稳定；如果延迟过高，降到 `detail=low` 或最长边 768；如果商品标签、接口文字、小规格图识别不足，再升到 `detail=high`。`doubao-seed-2-0-lite-260428` 当前不支持 Chat Completions 的 `response_format=json_object`，因此默认关闭 JSON mode，依靠提示词输出 JSON 后在本地解析和归一化。
...
运行时 Trace 会把 `retrieval_stack` 标成 `text_embedding(...)` 或 `hashing_vector`，商品 `match_reasons` 也会区分“语义向量”和“本地向量”。这样答辩时可以展示生产级路径和离线 fallback：有 embedding 服务时语义召回更强，无 key 或模型不可用时仍保证本地演示稳定。

热门检索还有一层 TTL/LRU 缓存：相同 query、结构化约束、limit 和向量后端身份会命中 `RetrievalCache`，直接返回深拷贝商品卡片，避免重复全表 BM25/向量重排。`/api/traces/{trace_id}` 的 `retrieval.cache_hit` 会标记命中状态，`/api/metrics` 和 `/admin/metrics` 会展示 `retrieval_cache_hit_rate`。
