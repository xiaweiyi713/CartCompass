...
1. 先在输入框输入 `油皮防晒` 或 `黑色手机`。
2. 不点击发送，直接上传一张商品图片。

预期：

- Agent 会提示正在融合图片和文字需求。
- 商品卡的推荐理由会包含 VLM 图像理解词、图片相似度、文本筛选词和融合排序依据。
- 如果配置了可选 VLM，推荐理由会显示识别出的品类、子类目、关键词或外观属性；未配置时会明确显示 VLM fallback。
- 如果本机安装了可选 CLIP 依赖，推荐理由会显示 CLIP 语义图像相似度；否则会明确显示轻量视觉 fallback。
