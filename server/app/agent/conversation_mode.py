...
class ConversationModeRouter:
    ...
    def _is_product_followup_question(self, compact: str, has_last_products: bool) -> bool:
        if not has_last_products:
            return False
        reference = any(term in compact for term in ["这款", "这个", "它", "第一款", "第二款", "第三款", "上一款"])
        fact_question = any(
            term in compact
            for term in [
                "评论",
        ...
        return reference and fact_question
    ...
    def _is_travel_buy_request(self, compact: str) -> bool:
        ...
        buy_terms = ["买什么", "带什么", "准备什么", "推荐", "配一套", "要带的东西", "应该买"]
    ...
    def _is_weather_query(self, compact: str) -> bool:
        return "天气" in compact and any(term in compact for term in ["今天", "明天", "现在", "下周", "怎么样", "如何", "冷吗", "热吗", "下雨"])
    ...
    def _is_weather_shopping_request(self, compact: str) -> bool:
        if "天气" not in compact and not any(term in compact for term in ["下雨", "冷吗", "热吗"]):
            return False
        return any(term in compact for term in ["买什么", "带什么", "准备什么", "推荐", "需要买", "需要带"])
